...
function cognitiveDistortionInterpretation(
  ...
  if (templateKey === "severe") {
    return `Заметные привычки мышления: ${highNames.join(", ")}. ${
      moderateNames.length > 0 ? `Также проявляются: ${moderateNames.join(", ")}. ` : ""
    }Дневник мыслей в приложении поможет замечать эти паттерны в моменте.`;
  }
  if (templateKey === "moderate") {
    return `Проявляются привычки мышления: ${moderateNames.join(
      ", ",
    )}. Осознание этих паттернов — первый шаг к изменениям.`;
  }
  return "Выраженных привычек мышления не выявлено. Ваш образ мыслей выглядит сбалансированным.";
}
...
function cognitiveDistortionRecommendation(
  ...
  if (recommendationKey === "severe") {
    return "У вас заметно выражено несколько привычек мышления. Дневник мыслей и практики в приложении помогут замечать эти паттерны и менять их с помощью фактов.";
  }
  if (recommendationKey === "moderate") {
    return "У вас есть склонность к некоторым привычкам мышления. Попробуйте вести дневник мыслей и замечать эти паттерны в повседневной жизни.";
  }
  return "Значимых привычек мышления не выявлено. Продолжайте замечать свои сбалансированные мысли.";
}
