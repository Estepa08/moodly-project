...
В репозитории только две постоянные ветки:

- **`main`** — интеграционная: сюда сливаются все фичи через PR.
- **`russia`** — прод-релизная: push в неё триггерит автодеплой на DockHost.

Процесс:

1. Фича → отдельная ветка → **PR в `main`**.
2. Релиз → **PR `main → russia`** → merge → автодеплой.
   Так как ветки стартуют из одной точки, релизные PR — чистые (fast-forward).

`main` защищена на GitHub (только через PR; force-push запрещён). Старое состояние `main`
хранит бэкап-метка `archive/old-main`.
