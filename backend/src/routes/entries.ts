import type { FastifyInstance } from 'fastify';
import { entryService } from '../services/entry.js';
import { createEntrySchema, updateEntrySchema, parseOrThrow } from '../lib/validation.js';
import { parsePagination, sendPaginated } from '../lib/pagination.js';

interface EntryCreateBody {
  id: string;
  parameterId: string;
  encryptedData: string;
}

interface EntryUpdateBody {
  encryptedData?: string;
}

export default async function entryRoutes(fastify: FastifyInstance) {
  fastify.get('/entries', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    const { parameterId, from, to, ...pagination } = request.query as {
      parameterId?: string;
      from?: string;
      to?: string;
      skip?: string;
      take?: string;
    };
    const result = await entryService.list({
      userId: request.userId,
      parameterId,
      from,
      to,
      ...parsePagination(pagination),
    });
    return sendPaginated(reply, result);
  });

  fastify.post<{ Body: EntryCreateBody }>(
    '/entries',
    { preHandler: [fastify.authenticate] },
    async (request) => {
      const data = parseOrThrow(createEntrySchema, request.body);
      return entryService.create({
        id: data.id,
        userId: request.userId,
        parameterId: data.parameterId,
        encryptedData: data.encryptedData,
      });
    },
  );

  fastify.get<{ Params: { id: string } }>(
    '/entries/:id',
    { preHandler: [fastify.authenticate] },
    async (request) => {
      return entryService.getById(request.params.id, request.userId);
    },
  );

  fastify.patch<{ Params: { id: string }; Body: EntryUpdateBody }>(
    '/entries/:id',
    { preHandler: [fastify.authenticate] },
    async (request) => {
      const data = parseOrThrow(updateEntrySchema, request.body);
      return entryService.update(request.params.id, request.userId, data);
    },
  );

  fastify.delete<{ Params: { id: string } }>(
    '/entries/:id',
    { preHandler: [fastify.authenticate] },
    async (request, reply) => {
      await entryService.delete(request.params.id, request.userId);
      reply.status(204);
    },
  );
}
