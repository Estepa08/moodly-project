...
export default async function reportRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      const entries = await reportService.getEntriesForPeriod(
        request.userId,
        report.periodFrom,
        report.periodTo,
      );
      ...
      doc
        .fontSize(10)
        .text(
          `Period: ${report.periodFrom.toISOString().slice(0, 10)} — ${report.periodTo.toISOString().slice(0, 10)}`,
        );
      doc.moveDown();
      ...
      for (const entry of entries) {
        doc
          .fontSize(11)
          .text(
            `${entry.createdAt.toISOString().slice(0, 10)} | ${entry.parameter.name}: ${entry.value}${entry.parameter.unit || ""}`,
          );
