...
export default function InsightBanner() {
  ...
  return (
    <div
      className={cn("relative flex items-center gap-3 p-4 rounded-xl bg-card shadow-neumorphic cursor-pointer", "transition-all duration-150 active:scale-[0.97]")}
      onClick={() => navigate(topRec.actionPath)}
      role="button"
      tabIndex={0}
      onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); navigate(topRec.actionPath); } }}
    >
      ...
      <button
        onClick={(e) => { e.stopPropagation(); setDismissed((prev) => new Set(prev).add(topRec.id)); }}
