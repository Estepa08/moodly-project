...
export function ChartTooltip({ active, payload, label, formatLabel, className }: ChartTooltipProps) {
  if (!active || !payload?.length) return null;
  return (
    <div
      className={cn("bg-card px-3 py-2 rounded-xl shadow-neumorphic-sm border border-border text-sm", className)}
      role="tooltip"
    >
