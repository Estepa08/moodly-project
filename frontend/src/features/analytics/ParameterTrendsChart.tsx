...
export default function ParameterTrendsChart({ trendData, paramNames, isLoading }: ParameterTrendsChartProps) {
  const { t } = useTranslation();
  const defaultParam = paramNames.includes("Mood") ? "Mood" : paramNames[0];
  const [visibleParams, setVisibleParams] = useState<Set<string>>(() => new Set(defaultParam ? [defaultParam] : []));
