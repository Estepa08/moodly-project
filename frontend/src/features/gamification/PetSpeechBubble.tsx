...
export default function PetSpeechBubble({
  ...
  return (
    <div
      ...
      aria-live="polite"
      className={cn(
        "relative rounded-2xl bg-card shadow-neumorphic-sm px-4 py-3 pr-10",
    ...
    >
      <span
        aria-hidden="true"
        className="absolute -bottom-1.5 right-[2.25rem] w-3.5 h-3.5 bg-card rotate-45"
      />
      <p className="text-sm font-semibold text-foreground leading-snug pr-8">{current.text}</p>
      ...
      <button
        ...
        className="absolute right-2 top-2 flex h-7 w-7 items-center justify-center rounded-full text-muted-foreground hover:text-foreground hover:bg-muted transition-colors cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
      >
