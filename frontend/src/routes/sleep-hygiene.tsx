...
export default function SleepHygienePage() {
  ...
  const { data: hygieneEntries } = useEntries(hygieneParam ? { parameterId: hygieneParam.id } : undefined);
  const { data: sleepEntries } = useEntries(sleepParam ? { parameterId: sleepParam.id } : undefined);
  const createEntry = useCreateEntry();

  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="text-center">
        <h2 className="text-xl font-semibold text-foreground font-serif">{t("sleepHygiene.title")}</h2>
